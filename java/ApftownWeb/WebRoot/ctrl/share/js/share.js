...
var getpath = function() {
    var w = window.location,
        ...
        wt = w.host,
        wp = w.protocol;
    var ui = ["/MiningCircle", "/Finance", "/upload", "/gl","/ApftownWeb","/Apftown"];
    var ui0 = ui[4],
        ui1 = ui[1];
    var path = "";
...
(function(_win) {
    ...
    var ShareTip = function(ele, opt) {
        var defaults = {
            "image": "http://www.apftown.com/img/login.jpg", //图片地址
            "title": "三亚亚太金融基金小镇apftown", //标题
            "link": "http://www.apftown.com/index.do?", //链接地址
            "vdesc": "更好的海棠湾等着你", //描述
    ...
    ShareTip.prototype = {
        ...
        create: function() {
            ...
            _BOM.each(sname, function(data, key) {
                ...
                var _aaa = ("<div class=\"SI_tool-btn wa-image-entity\" data-key=\"" + key + "\"  data-link=\"" + _link + "\"><img class=\"SI_img\" src=\"" + _self._path("img/" + key + ".png") + "\"><span class=\"SI_gap-top SI_line-clamp1\">" + data.name + "</span></div>");
                if(/cweixin|fweixin|alipay|qr|copy|email|qr|more/i.test(key) ){
                    if (/copy/.test(key)) {
                        _aaa = ("<div class=\"SI_tool-btn wa-image-entity copy"+_self.mitime+"\" id=sharepp data-clipboard-text=\"" + _conf.link + "\"data-key=\"" + key + "\"  data-link=\"" + _link + "\"><img class=\"SI_img\" src=\"" + _self._path("img/" + key + ".png") + "\"><span class=\"SI_gap-top SI_line-clamp1\">" + data.name + "</span></div>");
                        frag2.push(_aaa);
                    }else{
        ...
        _wxShre: function() {
            ...
            _open("//res.wx.qq.com/open/js/jweixin-1.0.0.js#tomi=js", function(data) {
                console.log(getpath()+"/weChatPay/share.do")
            	$.ajax({
                    type: 'get',
