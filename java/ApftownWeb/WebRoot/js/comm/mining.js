...
$("img").one("error", function(e){
     $(this).attr("src", $("base").attr("href")+"/img/comm/error.jpg");
});	
/*=================搜索提示框====================*/

	$(".search-con").on("click",function() {
		console.log("dasdsadasdasdsasd")
		$(".search-list").show();
...
$(function() {
	...
	$(".sel-ck").on("click", function(e) {
		...
		e.stopPropagation();
		var i=$(".sel-ck").index(this);
		$(".sel-ck .opt-ck").not($(".sel-ck .opt-ck").eq(i)).hide();
		$(this).find(".opt-ck").slideToggle(100)
		$(this).find("i").toggleClass("icon-caret-up");
	});
	$(".opt-ck").each(function() {
		$(this).on("click","li",function() {
			$(this).parent().parent(".sel-ck").find("input").val($(this).attr("value"));
...
$(".show").on("click",function(event) {
	...
	if (!$(target).closest(".modal-content").length > 0
			|| $(target).attr("class").indexOf("close-mod") != -1) {
		$(".show").hide()
	};
})
...
//修改删除操作的点击事件(拼接的只能用此方法)
function downClick(_this, e) {
	var e = e || window.event;
	e.stopPropagation();
	console.log($(_this).find(".opt-ck").is(":visible"))
	if ($(_this).find(".opt-ck").is(":visible")) {
		$(_this).find("i").removeClass("icon-caret-up");
		$(_this).find("i").addClass("icon-caret-down");
		$(_this).find(".opt-ck").hide();
	} else {
		$(_this).find(".opt-ck").show();
		$(_this).find("i").removeClass("icon-caret-down");
		$(_this).find("i").addClass("icon-caret-up");
	}
}
