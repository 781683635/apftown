...
<html>
  <head>
    ...
    <title>关于小镇-亚太金融(基金)小镇_基金注册_注册基金_基金小镇_金融小镇</title>
  ...
  <div class="wrap town">
	...
	<div class="swiper-container banner">
			...
			<div class="container clear">
				<div class="col-sm-3 col-md-2 pl-0 pr-0 pt-20 mt-20 intro-text">
					<div class="overflow-h">
						...
						<div class="gra-pri2">
							<h3 class="col-black pd-10 f-w f-16">关于小镇</h3>
							<div class="pb-5 gra-pri2 ml-10 mr-10"></div>
							
								<ul class="pt-10 pb-20 t-none f-14 subnav line-h-20" id="selCode">	
									<c:forEach var="listMap1"  items="${listMap1 }">
										<li class="mb-5">
										<span class="inline-block full-w pl-15 pt-5 pb-5 cur-p <c:if test='${propCode ==listMap1.code }'>cur-list</c:if>" onclick="changeCode(${listMap1.code })" value="${listMap1.code }" id="${list1.code }"><i class="icon-double-angle-right f-16 col-pri pr-5"></i>${listMap1.name } </span>
				...
				<div class="col-sm-6 col-md-5 col-lg-4  banner-info">
						...
						<div class="text pd-10">
							...
							<p class="f-18 pt-10 clear"><span class="pull-left col-white2 pt-5">宣传视频</span><img class="play pull-left img-30 ml-20" src="<%=path %>/img/video.png" alt="亚太金融（基金）小镇"></p>
		...
		<div class="video col-7 pd-0 mt-10">
			...
			<div class="closevd clear full-w pd-5">
				<img class="pull-right cur-p" src="<%=path %>/img/close.png" alt="亚太金融（基金）小镇">
			</div>
		</div>
	</div>
	<div class="container clear mt-20 ">
			...
			<div class="gra-pri2 pd-10" style="min-height:235px;">
				<c:forEach var="list0" items="${listMap0 }" varStatus="stat">	
					<c:if test="${list0.bk==0  }">
						<%-- <h3 class="f-16 col-black ">${list0.title }</h3> --%>
		...
		<div class="col-sm-10 product pr-0">	
			<h3 class="theme pd-5 f-16 mt-20"><span class="inline-block f-w pl-10 pd-10 propName">&nbsp;</span></h3>	
			<div class="bg-white clear pr-10">
				     		...
				     		<div class="line-h-15 mt-10">
								<a href="<%=path %>/n/i.do?no=${list2.no}" >
									<div class="img-out overflow-h">
										<img class="full-w transition1" src="${list2.img }" alt="图片加载失败" title="${list2.title }">
									</div>
									<p class="col-black pt-5 pb-10 f-13 text-overflow-1">${list2.title }</p>
								</a>
								<p class="text-line-2 h-30  let-spa-1 f-12 col-999">${list2.title2 }</p>
				...
				<form action="<%=path %>/t/index.do" id="pageForm" method="get">
							...
							<c:if test="${list201.bk==201 }">
								<div class="montext swiper-slide pos-r">
									<a class="inline-block transition2" href="<%=path %>/n/i.do?no=${list201.no}"> 
										...
										<p class="dis-n pos-b-0 pos-a rgba-70 col-ccc text-center line-h-15 col-12 text-overflow f-12
						     		    ">${list201.title }</p>
									</a>
								</div>
							</c:if>
